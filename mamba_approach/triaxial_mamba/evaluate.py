"""
Evaluation script for TriMamba-UNet MRI-to-CT synthesis.
...
Usage:
    ...
    python evaluate.py --data_dir /DATA/divyansh/mc_ddpm_data/brain_npy \
                       --checkpoint ./checkpoints_trimamba/trimamba_best.pth --tta
"""
...
def evaluate(args):
    ...
    # Load model (no deep supervision at inference)
    model = get_model('trimamba', base_ch=args.base_ch,
                      deep_supervision=False, use_checkpoint=False).to(device)

    ckpt = torch.load(args.checkpoint, map_location=device, weights_only=False)
    if 'model_state_dict' in ckpt:
        missing, unexpected = model.load_state_dict(ckpt['model_state_dict'], strict=False)
        if unexpected:
            print(f"[Warn] Ignoring {len(unexpected)} unexpected keys (e.g. deep-supervision heads): {unexpected[:2]}...")
        print(f"[Loaded] {args.checkpoint} (epoch {ckpt.get('epoch','?')})")
    else:
        missing, unexpected = model.load_state_dict(ckpt, strict=False)
        if unexpected:
            print(f"[Warn] Ignoring {len(unexpected)} unexpected keys: {unexpected[:2]}...")
    ...
    print("\n" + "=" * 60)
    print(f"[Results] TriMamba-UNet | TTA={'ON' if args.tta else 'OFF'} | "
    ...
    results_path = os.path.join(
        os.path.dirname(args.checkpoint), 'trimamba_test_results.txt')
    with open(results_path, 'w') as f:
        f.write(f"Model: TriMamba-UNet V2 (base_ch={args.base_ch})\n")
