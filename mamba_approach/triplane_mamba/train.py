"""
Training script for TriPlaneMamba-UNet MRI-to-CT synthesis.
...
Usage:
    ...
    python train.py --data_dir /DATA/divyansh/mc_ddpm_data/brain_npy \
                    --resume ./checkpoints_triplane/triplane_epoch150.pth
"""
...
def save_training_dashboard(history, mri, pred, target, epoch, save_dir):
    ...
    for i, (img, title) in enumerate([
        (mri_slice,    "Input MRI"),
        (pred_slice,   f"Predicted CT (ep {epoch})"),
...
def train(args):
    ...
    # ── LR Schedule ──
    # T_0=100: LR restart does not coincide with loss stage change at epoch 50
    scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=100, T_mult=2)

    # ── AMP (fp16 — bf16 not supported by torch.fft on all GPUs) ──
    ...
    if args.resume:
        ...
        print(f"[Resume] Continuing from epoch {start_epoch} | "
              f"Best PSNR so far: {best_val_psnr:.2f} | "
              f"LR: {optimizer.param_groups[0]['lr']:.2e}")
        del ckpt
        torch.cuda.empty_cache()

    print(f"\n[Config] model=TriPlaneMamba-UNet | epochs={args.epochs} | "
          f"batch={args.batch_size} | lr={args.lr} | "
          f"patch={args.patch_size} | d_state={args.d_state} | "
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if (epoch % 5 == 0 or epoch <= 5) and snapshot_data is not None:
            save_training_dashboard(
                history, *snapshot_data, epoch, visuals_dir)
        ...
        def _make_ckpt():
            return {
                ...
                'val_psnr':             val_psnr,
                'history':              history,
                # config stored so resume can verify compatibility
                'config': {
                    'base_ch':    args.base_ch,
                    'd_state':    args.d_state,
        ...
        if np.isfinite(val_psnr) and val_psnr > best_val_psnr:
            ...
            torch.save(_make_ckpt(),
                       os.path.join(args.save_dir, 'triplane_best.pth'))
            print(f"  --> Saved best (PSNR: {val_psnr:.2f}, MAE: {val_mae:.4f})")

        # ── Periodic checkpoint (every 50 epochs) ──
        if epoch % 50 == 0:
            torch.save(_make_ckpt(),
                       os.path.join(args.save_dir, f'triplane_epoch{epoch}.pth'))

    # ── Save log ──
    log_path = os.path.join(args.save_dir, 'triplane_training_log.txt')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='TriPlaneMamba-UNet MRI-to-CT Training')
    ...
    parser.add_argument('--d_state',     type=int,   default=16,
                        help='Mamba SSM state size (16 default, 32 for richer context)')
    parser.add_argument('--num_workers', type=int,   default=4)
    parser.add_argument('--save_dir',    type=str,
                        default='./checkpoints_triplane')
    parser.add_argument('--patch_size',  type=int,   nargs=3,
                        default=[32, 128, 128])
    parser.add_argument('--resume',      type=str,   default=None,
                        help='Path to triplane checkpoint to resume from')
