...
# CONFIG
parser = argparse.ArgumentParser(description="Evaluate Pix2Pix checkpoints on prepared 2D MR/CT slice CSVs.")
parser.add_argument("--config", default="./configs/pix2pix_test.yaml", help="Path to the Pix2Pix test YAML file.")
parser.add_argument("--fold", type=int, default=None, help="Optional single fold index to evaluate.")
...
cv = cfg['data']['cv']
fold_list = [args.fold] if args.fold is not None else list(range(cv))
