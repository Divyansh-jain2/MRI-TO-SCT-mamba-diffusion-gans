...
def test_4metrics(gen, loader, mse_metric, psnr_metric, device, outputs_dir,
    ...
    with torch.no_grad():
        for idx, (x, y, file_names) in enumerate(loop):
            ...
            loss2 = psnr_metric(y_fake, y)
            loss3 = vif_p(y_fake.float(), y.float())
            loss4 = _safe_ssim_value(y_fake, y)

            # statistics
            running_loss1 += loss1.item() * x.size(0)
            running_loss2 += loss2.item() * x.size(0)
            running_loss3 += loss3.item() * x.size(0)
            if torch.isfinite(loss4):
                running_loss4 += loss4.item() * x.size(0)
                valid_ssim_count += x.size(0)

    epoch_loss1 = running_loss1 / len(loader)
    epoch_loss2 = running_loss2 / len(loader)
    epoch_loss3 = running_loss3 / len(loader)
    epoch_loss4 = running_loss4 / max(valid_ssim_count, 1)
...
def test_pix2pix(gen, loader_test, mse_metric, psnr_metric, device, outputs_dir=None,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # SSIM (torchmetrics)
        ssim_val = _to_scalar(_safe_ssim_value(fake_x2, x2))
        ...
        for metric_name, metric_value in (
            ...
            if np.isfinite(metric_value):
                metric_sums[metric_name] += metric_value * bsz
                metric_counts[metric_name] += bsz
    ...
    for k, v in metric_sums.items():
        denom = max(metric_counts[k], 1)
        results[k] = v / denom if metric_counts[k] > 0 else float("nan")
...
def _safe_ssim_value(pred: torch.Tensor, target: torch.Tensor, data_range: float = 1.0) -> torch.Tensor:
    pred_32 = pred.float()
    target_32 = target.float()
    ssim_val = tm_ssim(pred_32, target_32, data_range=data_range)
    if torch.isfinite(ssim_val).all():
        return ssim_val
    return _global_ssim_fallback(pred_32, target_32, data_range=data_range)

def _global_ssim_fallback(pred: torch.Tensor, target: torch.Tensor, data_range: float = 1.0) -> torch.Tensor:
    ...
    var_pred = (pred_centered * pred_centered).mean(dim=dims)
    var_target = (target_centered * target_centered).mean(dim=dims)
    covar = (pred_centered * target_centered).mean(dim=dims)

    c1 = (0.01 * data_range) ** 2
    c2 = (0.03 * data_range) ** 2
    numerator = (2 * mu_pred * mu_target + c1) * (2 * covar + c2)
    denominator = (mu_pred.square() + mu_target.square() + c1) * (var_pred + var_target + c2)
    ssim_per_image = numerator / denominator.clamp_min(1e-12)
